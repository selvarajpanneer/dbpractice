...
public class RollAndCommit {
	...
	public static void main(String[] args) {
		Executable execute = new SqlUtil();// loose coupling
		execute.createConnection();// create database connection
		execute.createTable();// create table
		entryData(execute);
	}
	...
	private static void entryData(Executable execute) {
		...
		switch (input) {
		case 1:
			studentEntry(scanner, execute);
			break;
		case 2:
			employeeEntry(scanner, execute);
			break;
		default:
	...
	// perform other operations like commit, roll back, get data,etc;
	private static void rollAndCommit(Executable execute) {
		...
		case 5:
			...
			break;
		case 6:
			ArrayList<Student> studentList = execute.getStudentData();
			for (Student student : studentList) {
				System.out.println("student id: " + student.getId() + " student name: " + student.getName()
						+ " student age: " + student.getAge() + " student hometown: " + student.getHometown());
			}
			userFriendly(execute);
			break;
		case 7:
			ArrayList<Employee> employeeList = execute.getEmployeeData();
			for (Employee employee : employeeList) {
				System.out.println("employee id: " + employee.getId() + " employee name: " + employee.getName()
						+ " employee age: " + employee.getAge() + " employee hometown: " + employee.getHometown()
		...
		case 8:
			...
			while(true) {
				System.out.println("press 9 to create connection otherwise not implement any");
